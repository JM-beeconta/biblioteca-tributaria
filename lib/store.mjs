...
// Fusiona documentos nuevos contra los existentes por id, con un respaldo por source_url para
// reconocer un documento que cambió de id (p.ej. una renumeración). Ese respaldo por URL sólo es
// seguro cuando la URL identifica a un único documento: varios artículos BCN citan legítimamente
// la misma norma (misma source_url), y si se les aplicara el respaldo por URL, cada documento
// nuevo "robaría" la URL del anterior y lo borraría en cascada — por eso se excluyen las URLs
// que más de un documento comparte a propósito.
export function mergeDocuments(existing, incoming, { now = new Date().toISOString() } = {}) {
  const byId = new Map(existing.map((doc) => [doc.id, doc]));

  // Ambigua = más de un documento comparte esa URL dentro del mismo lado (existente o entrante).
  // Un documento existente y uno entrante con la misma URL (1:1) sí es el caso normal de
  // "cambió de id" y debe seguir resolviéndose por URL.
  function countUrls(docs) {
    const counts = new Map();
    for (const doc of docs) if (doc.source_url) counts.set(doc.source_url, (counts.get(doc.source_url) || 0) + 1);
    return counts;
  }
  const ambiguousUrls = new Set();
  for (const [url, count] of countUrls(existing)) if (count > 1) ambiguousUrls.add(url);
  for (const [url, count] of countUrls(incoming)) if (count > 1) ambiguousUrls.add(url);

  const byUrl = new Map();
  for (const doc of existing) {
    if (doc.source_url && !ambiguousUrls.has(doc.source_url)) byUrl.set(doc.source_url, doc);
  }

  const touched = new Set();
  for (const doc of incoming) {
    const byUrlMatch = doc.source_url && !ambiguousUrls.has(doc.source_url) ? byUrl.get(doc.source_url) : null;
    const old = byId.get(doc.id) ?? byUrlMatch;
    ...
    if (merged.source_url && !ambiguousUrls.has(merged.source_url)) byUrl.set(merged.source_url, merged);
  }

  return { documents: [...byId.values()], touched };
}

export function shardName(doc) {
  if (doc.source === 'BCN') return 'bcn.json';
  // Number(null) y Number('') son 0, y Number.isInteger(0) es true: sin este guard, un documento
  // sin año conocido caía silenciosamente en el shard "0.json" en vez de "sin-fecha.json".
  if (doc.year !== null && doc.year !== undefined && doc.year !== '' && Number.isInteger(Number(doc.year))) {
    return `${Number(doc.year)}.json`;
  }
