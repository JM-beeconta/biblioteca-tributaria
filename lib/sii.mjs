...
export async function crawlSii({ years, onDocument, log = console.log }) {
  ...
  for (const source of SOURCE_SETS) {
    ...
    for (const year of sourceYears) {
      ...
      const html = decodeBuffer(index.buffer, index.contentType);
  ...
  const shells = [...seenUrls.values()];
  log(`[SII] ${shells.length} documentos candidatos para ${years.at(-1)}-${years[0]}`);

  // Concurrencia deliberadamente baja para no sobrecargar los servicios públicos del SII.
  await mapWithConcurrency(shells, 3, async (shell) => {
    log(`[SII] Documento ${shell.type} ${shell.number ?? ''}/${shell.year}`);
    let extracted;
    try { extracted = await fetchDocumentText(shell.source_url); }
    catch (error) { log(`[SII] Error documento ${shell.source_url}: ${error.message}`); return; }
    if (!extracted) return;
