import { compact, decodeBuffer, extractAnchors, extractReferences, fetchResource, inferDate, pdfToText, sha256, stripHtml } from './utils.mjs';
  ...
  try {
    const response = await fetch(LIST_API, {
      ...
      headers: {
        ...
        'user-agent': 'BeecontaBibliotecaTributaria/0.3 (+https://github.com/JM-beeconta/biblioteca-tributaria)',
      },
...
function isPdf(response) {
  return /pdf/i.test(response.contentType)
    || /\.pdf(?:$|\?)/i.test(response.url)
    || response.buffer.subarray(0, 5).toString('latin1') === '%PDF-';
}

async function fetchDocumentText(url) {
  const response = await fetchResource(url, { retries: 2, delayMs: 650, timeoutMs: 25000 });
...
async function collectDynamic(source, year, indexUrl, html, shells, log) {
  ...
  const rows = await listDynamicOficios({ ...config, year }, log);
  ...
  for (const row of rows) {
    ...
    const key = blobId ? `blob:${blobId}` : downloadUrl;
    mergeShell(shells, key, {
      source: 'SII',
      type: 'oficio',
      year,
      number,
...
export async function crawlSii({ years, onDocument, log = console.log }) {
  ...
  for (const source of SOURCE_SETS) {
    ...
    for (const year of annualYears) {
      ...
      // Las páginas históricas 1994-2009 son HTML clásico; desde 2010 se prueba primero el índice moderno.
  ...
  // Dos descargas simultáneas como máximo para ser conservadores con los servicios públicos del SII.
  await mapWithConcurrency(allShells, 2, async (shell) => {
    ...
    const text = extracted?.text || shell.context || shell.summary || shell.legal || '';
