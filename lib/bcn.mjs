...
function textFromLeyChileJson(value) {
  ...
  return longest.length >= 4000 ? longest : [...new Set(pieces)].join('\n\n');
}

function printUrl(norm, versionDate = null) {
  const params = new URLSearchParams({ idNorma: String(norm.idNorma) });
  if (versionDate) params.set('idVersion', versionDate);
  return `https://www.bcn.cl/leychile/Navegar/imprimir?${params}`;
}

async function fetchPrintedNorm(norm, versionDate = null, log = console.log) {
  const url = printUrl(norm, versionDate);
  try {
    const response = await fetchResource(url, {
      retries: 3,
      delayMs: 350,
      timeoutMs: 35000,
      headers: { referer: `https://www.bcn.cl/leychile/navegar?idNorma=${norm.idNorma}` },
    });
    if (!response) return null;
    const raw = decodeBuffer(response.buffer, response.contentType);
    const text = stripHtml(raw);
    const articles = splitArticles(text);
    if (text.length < 3000 || articles.length < 10 || /este proceso demora demasiado/i.test(text)) {
      log(`[BCN] Impresión incompleta ${norm.code}${versionDate ? ` ${versionDate}` : ''}: ${text.length} caracteres, ${articles.length} artículos`);
      return null;
    }
    return { text, url: response.url, raw, format: 'html-print' };
  } catch (error) {
    log(`[BCN] Impresión ${norm.code}${versionDate ? ` ${versionDate}` : ''}: ${error.message}`);
    return null;
  }
}

async function fetchNorm(norm, versionDate = null, log = console.log) {
  return fetchPrintedNorm(norm, versionDate, log);
}
...
async function discoverVersions(norm, log = console.log) {
  ...
  const sources = [
    ...
    `https://www.bcn.cl/leychile/servicio/80/?idNorma=${norm.idNorma}&idParte=0`,
  ...
  return [...dates].filter((date) => date >= '1974-01-01' && date <= today).sort();
}
