...
export async function fetchResource(url, { retries = 3, delayMs = 450 } = {}) {
  let lastError;
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      const response = await fetch(url, {
        redirect: 'follow',
        headers: {
          'user-agent': 'BeecontaBibliotecaTributaria/0.1',
      ...
      lastError = error;
      if (attempt < retries) await sleep(delayMs * attempt * 2);
    }
  }
  throw lastError;
}
