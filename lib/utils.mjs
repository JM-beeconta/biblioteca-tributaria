...
export function extractAnchors(html = '', baseUrl) {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    let raw = html.slice(from, to);
    // El corte hacia atrás puede caer en medio de una etiqueta previa (p.ej. dejar sólo
    // el resto de un atributo: 9">) — si aparece un '>' antes que cualquier '<', ese
    // fragmento es la cola de una etiqueta cortada, no texto real: se descarta.
    const firstOpen = raw.indexOf('<');
    const firstClose = raw.indexOf('>');
    if (firstClose !== -1 && (firstOpen === -1 || firstClose < firstOpen)) raw = raw.slice(firstClose + 1);
    anchors.push({
      url,
      label: stripHtml(match[4]),
      context: stripHtml(raw),
    });
  }
  return anchors;
}
